...
Проект разработан в рамках Международного конкурса научно-технологических проектов **«Большие вызовы»** (направление: «Транспортно-логистические системы... / Искусственный интеллект...»).
...
3. **Система безопасности «Автостоп»:** Чтобы робот не врезался в ноги, алгоритм непрерывно анализирует ширину маркера. Как только она превышает 180 пикселей (~30 см до объекта), моторы экстренно останавливаются.
...
1. Склонируйте репозиторий на вашу Raspberry Pi:
   ```bash
   git clone [https://github.com/ezicvtumane/pi-smart-follower.git](https://github.com/ezicvtumane/pi-smart-follower.git)
   cd pi-smart-follower
Установите зависимости (камера Picamera2 должна быть включена в настройках ОС):
...
Bash
python3 main.py
Откройте интерфейс управления в браузере смартфона или ПК (устройства должны быть в одной Wi-Fi сети):
...
Создайте файл службы: sudo nano /etc/systemd/system/alicebot.service

Скопируйте в него содержимое файла alicebot.service из этого репозитория.

Включите автозагрузку командами:
...
📹 Тест следования за меткой (https://disk.yandex.ru/i/dgs2dP-XnfjGSw)

📹 Тест FPV и следование за Felis catus (https://disk.yandex.ru/i/KrElrjvzCOmklQ)


📝 Проект выполнен ученицей 9-го класса Ковалевой Алисой в рамках инженерно-исследовательской работы по направлению «Транспортно-логистические системы, морские, авиационные и беспилотные технологии», «Большие данные, искусственный интеллект, автоматизированные системы и информационная безопасность» для конкурса Большые вызовы.
